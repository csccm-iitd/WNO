...
for value in range(y_test.shape[0]):
    if value % 20 == 1:
        ...
        plt.imshow(y_test[value,:,:], label='True', cmap='Spectral')
        plt.title('Actual')
        plt.subplot(2,5, index+1+5)
        plt.imshow(pred.cpu().detach().numpy()[value,:,:], cmap='Spectral')
...
"""
# torch.save(model, 'model/model_wno_darcy2d')
# scipy.io.savemat('pred/pred_wno_darcy2d.mat', mdict={'pred': pred.cpu().numpy()})
# scipy.io.savemat('loss/train_loss_wno_darcy2d.mat', mdict={'train_loss': train_loss.cpu().numpy()})
# scipy.io.savemat('loss/test_loss_wno_darcy2d.mat', mdict={'test_loss': test_loss.cpu().numpy()})
