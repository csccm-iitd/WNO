...
class WNO2d(nn.Module):
    ...
    def get_grid(self, shape, device):
        ...
        gridy = torch.tensor(np.linspace(0, 3, size_y), dtype=torch.float)
...
for value in range(y_test.shape[0]):
    if value % 20 == 1:
        ...
        plt.imshow(y_test[value,:,:], label='True', cmap='seismic')
        plt.title('Actual')
        plt.subplot(2,5, index+1+5)
        plt.imshow(pred.cpu().detach().numpy()[value,:,:], cmap='seismic')
...
"""
# torch.save(model, 'model/model_wno_AC2d')
# scipy.io.savemat('pred/pred_wno_AC2d.mat', mdict={'pred': pred.cpu().numpy()})
# scipy.io.savemat('loss/train_loss_wno_AC2d.mat', mdict={'train_loss': train_loss.cpu().numpy()})
# scipy.io.savemat('loss/test_loss_wno_AC2d.mat', mdict={'test_loss': test_loss.cpu().numpy()})
