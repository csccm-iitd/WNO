...
-- This code is for performing predictions on pre-trained models for
   2-D Darcy equation in rectangular domain (time-independent problem).
"""
...
class WaveConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        dwt = DWT(J=4, mode='symmetric', wave='db4').to(device)
        x_ft, x_coeff = dwt(x)
...
model = torch.load('model/model_wno_2d_darcy_rect')
